...
6. **MANDATORY — complete this `<thinking>` block before writing any parameters:**
   ...
   - **冰饮参数重构（设计冰饮时必做）**：冰饮不能直接从热饮微调，必须按知识库第十节规则从头重构：
     - 热水比例从1:15-16降至 **1:10**（总热水150ml/15g），预留冰块稀释空间
     - 研磨度比热饮 **再细6-8格**（补偿水量减少30-40%）
     - 水温比热饮 **+2-3°C**（急冷锁香，但不得超过该豆萃取阈值上限）
     - 闷蒸固定 **45s**（不随豆子变化，冰冲一律延长）
     - 段数缩减至 **3段**（水少段少更高效），中段用spiral补偿萃取
     - 配方说明必须包含："分享壶预放100-120g冰块"
