...
来源：用户提供的 XBloom 官方对照表。**所有刻度换算必须先经此表，得到 Studio 档位基准后再做烘焙度/处理法/豆龄修正。**

| Approx. Particle (um) | XBloom Studio Grinder Level | XBloom Original Grinder Level | C40 Equivalent | Grind Description |
|---|---|---|---|---|
| 300um | 1 | / | 11 | Fine (细) |
| 400um | 10 | / | 15 | Medium Fine (中细) |
| 500um | 20 | / | 18 | Medium Fine (中细) |
| 600um | 30 | / | 22 | Medium (中) |
| 700um | 41 | 1 | 26 | Medium (中) |
| 800um | 50 | 10 | 30 | Medium-Coarse (中粗) |
| 900um | 60 | 20 | 33 | Medium-Coarse (中粗) |
| 1000um | 70 | 30 | 36 | Medium-Coarse (中粗) |
| 1100um | 80 | / | 40 | Coarse (粗) |

### 11.1 换算规则

- **量程**：XBloom Studio 研磨度 1–80（数值越小越细），与配方 API `grinderSize` 取值范围一致
- **优先用 C40**：烘焙商/包装上的参考研磨度最常见是 C40 刻度，直接查表得到 Studio 档位作为**基准**，再按豆子特性修正
- **初代机型**：初代刻度只有 700–1000um 段有对应值（1/10/20/30），其余粒度通过颗粒大小(um)中转换算
- **中间值**：表中未列出的刻度按相邻行线性近似（如 C40 20 格 ≈ 600um ≈ Studio 30）
- **修正顺序**：C40/初代/um → Studio 档位基准 → 烘焙度修正（1.2 节）→ 处理法/豆龄修正（7.x 节 + beans 技能新鲜度规则）→ 冰饮再细 6-8 格（10.2 节）

### 11.2 使用时机

- 豆库中 `referenceGrind` 字段记录了参考研磨度（如 "C40 18"、"800um"）时，出配方必须先换算
- 用户口头或图片提供参考研磨度（C40 刻度、初代刻度、颗粒 um）时，同样先换算
- 示例：浅烘水洗豆烘焙商建议 C40 22 → 查表对应 Studio 30 → 作为研磨度基准，再按知识库规律微调
